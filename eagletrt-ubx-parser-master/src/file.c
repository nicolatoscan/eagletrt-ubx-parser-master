...
int isjson(const char *t){		//identifica se il config.json è settato su cvs
  	int r=0;
	if (strcmp(t,json)==0){
		r=1;
		return r;
...
int iscsv(const char *t){		//identifica se il config.json è settato su cvs
  	int r=0;
	if (strcmp(t,csv)==0){
		r=2;
		return r;
...
char *matchubx(const char *n){			//ricostruisce nome file ubx
	char i[100]="inputs/";
	char *u=".ubx";
	char a[100];
...
char *matchtype(const char *n, int t){		//ricostruisce nome file di output
	char i[100]="outputs/";
	char *j1="json/";
	char *j2=".json";
	char *c1="csv/";
